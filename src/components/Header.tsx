    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <div className="flex items-center justify-between w-full md:w-auto">
          <Link href="/">
            <span className="text-2xl md:text-xl lg:text-3xl font-bold text-amber-500">
              MoviesHub
            </span>
          </Link>
        ...
        <motion.div className="relative w-full md:w-1/3 md:mx-8 hidden md:block">
          <input
            ...
            onChange={(e) => {
              ...
              setSuggestions([]);
              setIsSearchOpen(false); // close dropdown + revert icon to Search on any edit
            }}
          />
          <button
          ...
          <AnimatePresence>
            {isSearchOpen && (
              <motion.div
                className="absolute left-0 top-full mt-1 w-full bg-[#18181b] border border-gray-500 rounded-lg shadow-lg z-50"
              ...
              >
                ...
                suggestions.length > 0 ? (
                  suggestions.map((item: any) => (
                    <Link key={`${item.media_type}-${item.id}`} href={"/"}>
                      <div className="flex items-center gap-2 p-2 hover:bg-[#252525] rounded-lg cursor-pointer">
                        <Image
                          ...
                          width={42}
                          height={48}
                          className="w-15 aspect-2/3 object-cover rounded"
                        />
        ...
        <nav className="hidden md:flex md:items-center md:space-x-6">
          {navLinks.map((link) => (
            <Link
              key={link.name}
              href={link.path}
              className={`text-xs sm:text-base font-medium relative text-white ${pathname === link.path ? "text-white" : "hover:text-white/70"}`}
            >
              {link.name}
